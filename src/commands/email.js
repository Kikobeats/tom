...
module.exports = config => {
  ...
  const email = async opts => {
    ...
    const templateProps = pick(compiledTemplate, ['body', 'bcc', 'cc', 'from', 'subject'])
    const optsProps = pick(opts, ['body', 'bcc', 'cc', 'from', 'subject', 'attachments'])
    ...
    const info = await transporter.sendMail({from, to, cc, bcc, attachments, subject, text, html})

    const logProps = pick(mailOpts, ['from', 'subject', 'cc', 'bcc', 'attachments'])
